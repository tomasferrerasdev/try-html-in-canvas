...
A Chrome extension that snapshots the current web page using the **experimental [`drawElement()` Canvas API](https://github.com/WICG/html-in-canvas)** and renders it through a user-supplied **WebGL fragment shader**.

You get a popup with a GLSL editor (syntax highlighted, with presets for blur, swirl, invert, chromatic aberration, wave, pixelate) and a live overlay on the active tab.
...
6. In the popup: pick a preset or edit the GLSL, hit **Apply**. Hit **Remove** to tear it down.
...
- `chrome.scripting.executeScript` injects an in-page function (in the `MAIN` world) that:
  ...
  3. Uploads that canvas as a WebGL2 texture and renders a fullscreen triangle using your fragment shader.
